...
from statfunc import Cluster_Perm_Test, find_latency
    ...
    for dims in ['age', 'emotion', 'gender', 'race']:
        ...
        if not fname_tmp.exists():
            ...
            for i, dimdiff in enumerate([rr + '_diff' for rr in other_dim]):
                _tc = rbox[__type[0] + '_rsa_val'] - rbox[__type[1] + '_rsa_val']  # [n_subj, 1001, 3]
                _tc = _tc[:, :, i]
        ...
        for tp in range(1001):
            for subj in range(15):
                ...
                for i, cond in enumerate(['AA', 'AB', 'BB', 'BA']):
                    ...
                    _tmp1[i, :] = rankdata(_tmp1[i, :], method='average', nan_policy='omit')
                data = {
        ...
        for cond in abl:
            ...
            for cond2 in abl:
                if abl.index(cond2) > abl.index(cond):
                    _, _, ps = Cluster_Perm_Test(eval(f'{cond}_all'), eval(f'{cond2}_all'))
                    psig[tt][f'Count: {cond}-{cond2}'] = f'{ps.sum()} different timepoints...'
...
if not fname_low_lev.exists():
    ...
    for t in range(64):
        ...
        x, y, w, h = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5)[0]
        center_x = x + w // 2
        center_y = y + h // 2
        pic_size = gray.shape[0]
        _luminance[t] = gray.mean()
        _contrast[t] = np.sqrt(np.sum((gray - gray.mean()) ** 2) / (pic_size * pic_size))
    ...
    a = pose_estimation.batch_pose_detect('./pose/pic_nobg')
