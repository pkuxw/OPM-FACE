...
def GenRDM(picROOT: Path = Path("./pose/pic/")) -> Box:
    ...
    RDM = Box()
    RDM.rdm_lpips = _rdm
    RDM.rdm_race = 1.0 - np.array(
        (([1] * 32 + [0] * 32) * 32 + ([0] * 32 + [1] * 32) * 32) * 1
    ).reshape((64, 64))
    RDM.rdm_gender = 1.0 - np.array(
        (([1] * 16 + [0] * 16) * 32 + ([0] * 16 + [1] * 16) * 32) * 2
    ).reshape((64, 64))
    RDM.rdm_age = 1.0 - np.array(
        (([1] * 8 + [0] * 8) * 32 + ([0] * 8 + [1] * 8) * 32) * 4
    ).reshape((64, 64))
    RDM.rdm_emotion = 1.0 - np.array(
        (([1] * 4 + [0] * 4) * 32 + ([0] * 4 + [1] * 4) * 32) * 8
    ).reshape((64, 64))
...
if __name__ == "__main__":
    ...
    if not fname_svm.exists():
        ...
        rsa_val = rsa(
            dsm_data, dsm_model, metric="partial-spearman", n_jobs=-1, verbose=True
        )
    ...
    def swap_2(
        rdm: np.ndarray, dim: str = "race", idx: int = 0
    ...
    for dim in ["race", "gender", "age", "emotion"]:
        ...
        if not fname_data.exists():
            ...
            for type_id, type in enumerate(__type):
                _d = np.zeros(
                    (data.shape[0], data.shape[1], 32, 32)
                )  # (21, 1001, 32, 32)
                ...
                rsa_val = rsa(
                    _data, _model, metric="partial-spearman", n_jobs=-1, verbose=True
                )
    ...
    if not fname_ctg.exists():
        ...
        for t in range(501):
            for i, f in enumerate(SUBJ):
                _mat[i, :, t, :] = jl.load(ROOT / "ctg" / f / f"res-tp{t:04d}.jl")[
                    :, 0, :
                ]
    ...
    if not fname_rel.exists():
        ...
        for part, part2 in zip(
            ["AA", "BB", "AB", "BA"], ["A to A", "B to B", "A to B", "B to A"]
        ):
            dat[part] = np.zeros((15, 1001, 64, 64))
            for idx, subj in enumerate(SUBJ_15):
                for tp in range(1001):
                    dat[part][idx][tp] = jl.load(
                        ROOT_REL / part2 / subj / f"tp{tp:04d}.jl"
                    )
